        ...
        if request.method == 'POST':  # runs if form has been filled in else it will render an empty form
            ...
            if form.is_valid():
                ...
                answer = Answer(question=question, user=request.user,
                                answer_text=answer_text, answer_date=answer_date)  # Link answer to user
...
def user_answers(request):
    ...
    # retrieve all the answers made by the user
    user_answers = Answer.objects.filter(user=user)
    # create a context dictionary with the user answers
    context = {'user_answers': user_answers}
    # render the user_answers.html template with the context dictionary
...
def organise_answers(request):
    ...
    try:
        user = request.user
        subquery1 = Answer.objects.filter(
            user=user, question__question_identifier=1).order_by('-answer_date')[:1]
        subquery2 = Answer.objects.filter(
            user=user, question__question_identifier=2).order_by('-answer_date')[:1]
        subquery3 = Answer.objects.filter(
            user=user, question__question_identifier=3).order_by('-answer_date')[:1]
...
def most_recent_answer(request):
    ...
    most_recent_answer = Answer.objects.filter(question__question_identifier=1, user=user).latest(
        'answer_date')  # geta answer object using current useer and  custom question id number
